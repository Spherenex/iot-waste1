...
import './SmartIndoorFarming.css'; // Import your CSS file for styling
...
const sheetId = "1BiMyeNK6EGgRW4PK0vUj8-193dxSCmhHXTXTvhatmxs"; // New sheet ID
...
const AgriculturalData = ({ data, currentTemp }) => {
  ...
  const cards = [
    ...
    {
      title: 'Soil',
      icon: 'https://img.icons8.com/fluency/48/soil.png',
      values: [{ label: '', value: data.soil === 'dry' ? 'wet' : data.soil === 'wet' ? 'dry' : data.soil }],
      className: 'card card-soil',
      gridArea: 'soil',
    },
    {
      title: 'Turbidity',
...
const DataTable = ({ data, currentTemp }) => {
  ...
  const parameters = [
    ...
    { label: 'Soil Status', value: data.soil === 'dry' ? 'wet' : data.soil === 'wet' ? 'dry' : data.soil },
